# clip/clip_rewarded_sac.py
# =====================
# CLIPRewardedSAC: 基于SAC的VLM奖励强化学习算法实现
# - 集成CLIP奖励，支持端到端VLM-RL训练
# 输入：环境、配置
# 输出：训练好的模型
# 主要功能：
#   - 继承SAC，重载回放缓冲区、奖励计算、模型保存/加载等
#   - 支持CLIP/VLM奖励的自动计算与融合
#   - 兼容Stable-Baselines3的训练与推理流程
    ...
    def _compute_clip_rewards(self):
        ...
        if not self.inference_only:
            if replay_buffer_pos - env_episode_timesteps >= 0:
                ...
                # n = replay_buffer_pos
                n = env_episode_timesteps - replay_buffer_pos
                self.replay_buffer.rewards[-n:] = rewards[:n]
                self.replay_buffer.rewards[:n] = rewards[-n:]
