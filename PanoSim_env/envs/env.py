...
class PanoSimCamera():
    def __init__(self):
        self.data = np.zeros((192, 192, 6), dtype=np.uint8)

class PanoSimEnv(gym.Env):
    metadata = {
    ...
    def __init__(
        ...
        self.vehicle = PanoSimEgo()
        self.camera = PanoSimCamera()
        self.observation = self.camera.data
        self.current_waypoint_index = 0
        self.route_waypoints = []
        for i in range(15):
            self.route_waypoints.append([CarlaWayPoint()])
        ...
        self.bev_data = np.zeros((517, 517, 3), dtype=np.uint8)
        self.bev_data_buffer = np.zeros((517, 517, 3), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        info = {
            ...
            "centering_factor": calc_value,
            "angle_factor": calc_value,
            "distance_std_factor": calc_value,
